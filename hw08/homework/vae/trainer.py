...
class Trainer:
    ...
    def __init__(self, model, train_loader, test_loader, optimizer,
                 loss_function, device='cpu', log_dir=None):
        self.model = model
    ...
    def test(self, epoch, batch_size, log_interval, n_plot):
        self.model.eval()
        test_epoch_loss = 0
        ...
        self.writer.add_scalar(tag='data/test_epoch_loss',
                               ...
                               global_step=epoch)
        self.plot_generated(epoch, n_plot)

    def plot_generated(self, epoch, n_plot):
        pix = next(iter(self.test_loader))[0].to(self.device)[:n_plot]
        decoded, _, _ = self.model(pix)
        all = torch.cat([pix, decoded])
        grid = vutils.make_grid(all, normalize=True, scale_each=True)
